ui <- shinyUI(fluidPage(theme="style/kappa.css", # prefix 'style' to locate the CSS file: cf. StartMMD.R
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      conditionalPanel(condition="input.typeData == 'raw'", # this panel will be displayed for a "raw dataset" only
        fluidRow( # create a grid of widgets with 2 columns of equal widths
          column(6,
            checkboxInput("colNamesRaw", label="Variable names in first row", value=TRUE),
      ...
      conditionalPanel(condition="input.typeData == 'table'", # this panel will be displayed for a "table of n's and frequencies" only
        fluidRow(
          column(6,
      ...
      actionButton("loadData", "Load dataset", icon=icon("file-upload")), # button to load the data (server.R is waiting a mouse click here to begin)
      br(),
      br(),
      ...
      fluidRow(
        column(6,
          ...
          conditionalPanel(condition="input.exclusionStrategy == 'excludeNOMD'", # this panel will be displayed only if the button "overall MD" is active
