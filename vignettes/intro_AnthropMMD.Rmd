...
The MMD is a dissimilarity measure among groups of individuals described by binary (presence-absence) traits. The MMD formula only requires to know the sample sizes and relative trait frequencies within each group: providing the raw individual data is not mandatory. 

Typically, the user will register the data in one of the the two following formats.
...
Sometimes, in particular if the data were extracted from research articles rather than true exprimentation, the user will only know the sample sizes and absolute trait frequencies in each group. As previously stated, those data are perfectly sufficient to computed MMD values. Here is an example of such a table:
```{r echo=FALSE}
absolute_freqs <- binary_to_table(data = toyMMD, relative = FALSE)
```
```{r}
absolute_freqs
```
